...
//       )
//       .join("")
//     : `<tr><td colspan="6" style="text-align:center;color:var(--text3);padding:20px">Brak zaliczek</td></tr>`;
// }
...
//       })
//       .join("") ||
//     `<tr><td colspan="6" style="text-align:center;color:var(--text3);padding:20px">Brak wniosków</td></tr>`;
// }
...
//     data = getFilteredAdvances().map((a) => ({
//       Pracownik: a.full_name,
//       Login: a.login,
//       Obiekt: a.facility_name || "—",
//       "Data wniosku": new Date(a.requested_at).toLocaleDateString("pl"),
//       "Godziny (mies.)": a.hours_this_month > 0 ? parseFloat(a.hours_this_month).toFixed(1) : "0",  // ← ДОБАВЬ
//       Status: a.status,
//     }));
...
//       });
//       row["Suma"] = total > 0 ? total.toFixed(1) : "0";
//       return row;
//     });
...
//   const received = filtered.filter((w) => w.received).length;
//   const missing = filtered.filter((w) => !w.received).length;

//   document.getElementById("tabeleCount").textContent = filtered.length;
//   document.getElementById("tabeleTotal").textContent = filtered.length;
