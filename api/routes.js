...
// Условие исключения TEST-работников из СЧЁТЧИКОВ И СУММ.
// В списках (workers/all, workers, tabele, advances, history) TEST остаются видимыми.
const NOT_TEST = `login NOT LIKE 'TEST_%'`;
  ...
  try {
    const res2 = await db.query(
      `SELECT f.*, f.group_name, COUNT(w.id) AS worker_count
       FROM facilities f
       LEFT JOIN v_worker_current w ON w.facility_id = f.id
...
router.get("/stats", requireAuth, async (req, res) => {
  try {
    ...
    // TEST-работники не входят ни в один счётчик статистики
    let facWhere = `w.status = 'pracuje' AND w.${NOT_TEST}`;
    const facParams = [];
    ...
    let advWhere = `a.status = 'pending' AND w.${NOT_TEST}`;
    const advParams = [];
    if (facility_id) {
...
router.get("/advances", requireAuth, async (req, res) => {
  try {
    const allowedFacilities = await getFacilityFilter(req.coordinator);
    let query = `
      SELECT a.*, w.full_name, w.login, w.bhp_date, f.name AS facility_name,
        COALESCE((
          SELECT SUM(h.hours)
          FROM hours_log h
...
router.get("/workers/stats", requireAuth, async (req, res) => {
  try {
    ...
    const [total, newWeek, leavingWeek, leavingMonth, startedWeek] = await Promise.all([
     ...
     WHERE w.status = 'pracuje' AND w.${NOT_TEST} ${facFilter}`,
        params,
      ),
      db.query(
        `SELECT COUNT(*) FROM v_worker_current w
    ...
    res.json({
      ...
      data: {
        ...
        started_this_week: parseInt(startedWeek.rows[0].count),
      },
    });
  } catch (err) {
    res.status(500).json({ ok: false, error: err.message });
...
router.get("/workers", requireAuth, async (req, res) => {
  try {
    ...
    // TEST-працівники ЛИШАЮТЬСЯ у списку (видимі), але фронтенд
    // не враховує їх у сумах (див. hours.js).
...
// TEST-работники исключены из всех агрегатов аналитики (JOIN workers wk)
router.get("/analytics/chart", requireAuth, async (req, res) => {
  try {
    ...
    const [hired, fired, moved, urlop, l4, rezygnacja] = await Promise.all([
      ...
      db.query(`
    SELECT h.bhp_date::date AS period, COUNT(DISTINCT h.worker_id) AS cnt
    FROM worker_facility_history h
    JOIN workers wk ON wk.id = h.worker_id AND wk.${NOT_TEST}
    WHERE h.bhp_date BETWEEN $1::date AND $2::date
      AND h.status::text NOT IN ('zwolniony','rezygnacja')
      ${facFilter}
    GROUP BY h.bhp_date::date ORDER BY h.bhp_date::date
  `, baseParams),
      // fired — звільнені по last_work_date
      db.query(`
      SELECT h.last_work_date::date AS period, COUNT(DISTINCT h.worker_id) AS cnt
      FROM worker_facility_history h
      JOIN workers wk ON wk.id = h.worker_id AND wk.${NOT_TEST}
      WHERE h.last_work_date BETWEEN $1::date AND $2::date
        ...
        ${facFilter}
      GROUP BY h.last_work_date::date ORDER BY h.last_work_date::date
    `, baseParams),
      // moved
      db.query(`
    SELECT h.bhp_date::date AS period, COUNT(DISTINCT h.worker_id) AS cnt
...
// POST /api/advances/batch  { ids: [1,2,3], action: "approve" | "reject" }
router.post("/advances/batch", requireAuth, async (req, res) => {
  try {
    ...
    const params = [newStatus, req.coordinator.coordinator_id, ids];
    let facFilter = "";
    if (allowedFacilities !== null) {
      if (!allowedFacilities.length)
        return res.status(403).json({ ok: false, error: "Brak obiektów" });
      params.push(allowedFacilities);
      facFilter = `AND a.worker_id IN (
        SELECT id FROM v_worker_current WHERE facility_id = ANY($4)
      )`;
    }

    const result = await db.query(
      `UPDATE advances a
       SET status = $1::advance_status, approved_by = $2, approved_at = now()
       WHERE a.id = ANY($3)
         AND a.status = 'pending'
         ${facFilter}
       RETURNING a.id, a.worker_id`,
      params,
    );

    res.json({ ok: true, updated: result.rowCount, ids: result.rows.map((r) => r.id) });

    // Telegram-сповіщення (не валимо відповідь, якщо щось не так)
    (async () => {
      for (const row of result.rows) {
        try {
          const wq = await db.query(
            `SELECT telegram_chat_id, lang FROM workers WHERE id = $1`,
            [row.worker_id],
          );
          const w = wq.rows[0];
          if (!w || !w.telegram_chat_id) continue;
          const { T } = require("../bot/i18n");
          const session = { lang: w.lang || "uk" };
          ...
          await new Promise((r) => setTimeout(r, 300)); // проти ліміту Telegram
        } catch (tgErr) {
          console.error("Telegram notify failed (batch advances):", tgErr.message);
        }
      }
    })();
  } catch (err) {
...
// POST /api/day-off/batch  { ids: [...], action: "approve"|"reject" }
router.post("/day-off/batch", requireAuth, async (req, res) => {
  try {
    const { ids, action } = req.body;
    if (!Array.isArray(ids) || !ids.length || !["approve", "reject"].includes(action))
      return res.status(400).json({ ok: false, error: "Bad request" });
    ...
    (async () => {
      for (const row of result.rows) {
        try {
          ...
          if (!w || !w.telegram_chat_id) continue;
          const emoji = newStatus === "approved" ? "✅" : "❌";
          const statusText = newStatus === "approved" ? "підтверджено" : "відхилено";
          await fetch(
            `https://api.telegram.org/bot${process.env.BOT_TOKEN}/sendMessage`,
            {
              method: "POST",
              ...
              body: JSON.stringify({
                chat_id: w.telegram_chat_id,
                text: `${emoji} Ваша заявка на вихідні ${statusText}.`,
                parse_mode: "Markdown",
              }),
            },
          );
          ...
          console.error("Telegram notify failed (batch dayoff):", tgErr.message);
        }
      }
    })();
  } catch (err) {
