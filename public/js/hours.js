...
// TEST-працівники: видимі у списках, але НЕ враховуються
// в жодних сумах годин і лічильниках людей
function isTestWorker(w) {
  return /^TEST_/i.test(w?.login || "");
}
...
function renderTabs(workers) {
  ...
  workers.forEach((w) => {
    ...
    // TEST видимі, але не входять у лічильники людей
    if (!isTestWorker(w)) {
      facs[fid].count++;
      groups[gname].count++;
    }
  ...
  const totalNonTest = workers.filter((w) => !isTestWorker(w)).length;
        ...
        <div id="groupDropMenu" style="display:none;position:absolute;top:36px;left:0;background:var(--surface2);border:1px solid var(--border);border-radius:8px;min-width:260px;max-height:400px;overflow-y:auto;z-index:9999;box-shadow:0 8px 24px rgba(0,0,0,.4);padding:6px 0;">
          ...
          <div id="groupList">
            <div onclick="selectGroup('');toggleGroupDrop()" style="padding:7px 14px;font-size:12px;cursor:pointer;display:flex;justify-content:space-between;color:${!currentGroup ? 'var(--accent)' : 'var(--text2)'};background:${!currentGroup ? 'rgba(59,130,246,.08)' : 'transparent'}">
              ...
              <span style="font-family:var(--mono);font-size:10px;color:var(--text3)">${totalNonTest}</span>
            </div>
...
function renderSummaryTable(workers) {
  ...
  workers.forEach((w) => {
    ...
    // TEST не входять ні в кількість людей, ні в суми годин
    if (isTestWorker(w)) return;
    facMap[fid].workers++;
    (w.hours || []).forEach((h) => {
...
function renderWorkersTable(workers) {
  ...
  const dayHeaders = Array.from({ length: daysInMonth }, (_, i) => {
    const d = i + 1;
    const isFuture = d > todayDay;
    return `<div class="day-cell" style="color:${isFuture ? 'var(--text3)' : 'var(--text2)'};font-size:9px;font-weight:600;background:var(--surface2);border:none">${String(d).padStart(2, "0")}</div>`;
  }).join("");

  const headerRow = `<tr style="border-bottom:1px solid var(--border)">
    <td style="padding:6px 14px;font-size:10px;color:var(--text3);font-family:var(--mono);letter-spacing:1px;background:var(--surface2)"></td>
    <td style="padding:6px 4px;background:var(--surface2)"></td>
    <td style="background:var(--surface2)"><div class="day-cells">${dayHeaders}</div></td>
 ...
 tbody.innerHTML = headerRow + slice.map((w) => {
    ...
    const testMark = isTestWorker(w)
      ? `<span style="font-size:9px;font-family:var(--mono);color:var(--text3);border:1px solid var(--border);border-radius:8px;padding:0 5px;margin-left:6px">TEST</span>`
      : "";

    return `<tr${isTestWorker(w) ? ' style="opacity:.65"' : ''}>
      <td><div class="worker-name">${w.full_name}${testMark}</div><div class="worker-id">#${w.login}</div></td>
      <td>${badge}</td>
